...
#
# Create a custom Jenkins agent pod 
#
oc new-build  -D $'FROM docker.io/openshift/jenkins-agent-maven-35-centos7:v3.11\n
      USER root\nRUN yum -y install skopeo && yum clean all\n
      USER 1001' --name=jenkins-agent-appdev -n a247-jenkins
...
- kind: "BuildConfig"
  ...
  spec:
    source:
      type: "Git"
      git:
        uri: "https://github.com/dqueeney/Openshift/blob/master/openshift-tasks/"
    strategy:
      type: "JenkinsPipeline"
      jenkinsPipelineStrategy:
        jenkinsfilePath: Jenkinsfile
kind: List
metadata: []" | oc create -f - -n a247-jenkins
